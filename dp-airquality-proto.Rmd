...
``` {r airquality-api-request-confirm}
DT::datatable(aq_df)
```
...
``` {r airquality-shiny-global, eval = FALSE}
# 0. 환경설정 -----
library(tidyverse)
library(httr)
library(rvest)
...
``` {r airquality-shiny-ui, eval = FALSE}
library(shiny)
...
``` {r airquality-shiny-server, eval = FALSE}
